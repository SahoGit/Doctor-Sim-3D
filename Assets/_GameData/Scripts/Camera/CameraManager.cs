...
public class CameraManager : MonoBehaviour {
	...
	void Update() {
		...
		// Mouse:
		angleH += Mathf.Clamp(RotationPanel.TouchDist.x, -1, 1) * horizontalAimingSpeed;
		angleV += Mathf.Clamp(RotationPanel.TouchDist.y, -1, 1) * verticalAimingSpeed;
		
        // Joystick:
		angleH += Mathf.Clamp(Character.myJoystick.Horizontal, -1, 1) * 60 * horizontalAimingSpeed * Time.deltaTime * 0.5f;
		// angleV += Mathf.Clamp(Character.myJoystick.Vertical, -1, 1) * 60 * verticalAimingSpeed * Time.deltaTime;
	...
	{
		float playerFocusHeight = playerColliderHeight * 0.75f;
